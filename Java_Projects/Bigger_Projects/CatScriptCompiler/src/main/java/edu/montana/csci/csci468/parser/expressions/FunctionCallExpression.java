...
public class FunctionCallExpression extends Expression {
    ...
    public Object evaluate(CatscriptRuntime runtime) {
       FunctionDefinitionStatement function = (FunctionDefinitionStatement) runtime.getValue(name);
       List<Object> val = new ArrayList<>();
       for(Expression argument: arguments){
           val.add(argument.evaluate(runtime));
       }

       return function.invoke(runtime, val);
    }
    ...
    public void compile(ByteCodeGenerator code) {
        ...
        code.addVarInstruction(Opcodes.ALOAD, 0); // Load 'this' for INVOKEVIRTUAL

        FunctionDefinitionStatement functionDefinitionStatement = getProgram().getFunction(name);

        for (int i = 0; i < arguments.size(); i++) {
            Expression arg = arguments.get(i);
            CatscriptType paramType = functionDefinitionStatement.getParameterType(i);
            CatscriptType argType = arg.getType();

            arg.compile(code);

            if ((argType.equals(CatscriptType.INT) || argType.equals(CatscriptType.BOOLEAN)) && paramType.equals(CatscriptType.OBJECT)) {
                box(code, argType);
            }
        }
