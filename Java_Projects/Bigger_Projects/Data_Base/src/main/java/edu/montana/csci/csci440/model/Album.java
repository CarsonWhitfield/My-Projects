...
public class Album extends Model {
    ...
    public static List<Album> getForArtist(Long artistId) {
        if (artistId ==  null){
            throw new IllegalStateException("Artist ID cannot be null");
        }
        ...
        try (Connection connect = DB.connect();
            ...
            stmt.setLong(1, artistId); // Set the artist ID in the query

            List<Album> result = new ArrayList<>();
            ResultSet resultSet = stmt.executeQuery();
            while (resultSet.next()) {
                result.add(new Album(resultSet)); // Assuming Album(ResultSet) constructor
            }
            return result;
        } catch (SQLException e) {
            throw new RuntimeException("Error retrieving albums for artist with ID: " + artistId, e);
        }
    }
    ...
    public void delete() {
        try (Connection conn = DB.connect();
            ...
            throw new RuntimeException("Error deleting album with ID: " + this.albumId, sqlException);
        }
    }
